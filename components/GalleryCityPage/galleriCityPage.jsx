...
export const GalleryCityPage = ({ id }) => {
  ...
  useEffect(() => {
    ...
    if (country?.toLowerCase() == "russia") {
      russia?.forEach((element) =>
        element.name === id ? (list = [...list, element.cover_image_url]) : null
      );
      ...
      italia?.forEach(
        (element) =>
          (list = [
            ...list,
            element.cover_image_url != list[0] && element.cover_image_url,
          ])
      );
    } else if (country?.toLowerCase() == "francia") {
      francia?.forEach((element) =>
        element.name === id ? (list = [...list, element.cover_image_url]) : null
      );
      francia?.forEach(
        (element) => (list = [...list, element.cover_image_url])
      );
